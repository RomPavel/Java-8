...
public class Staff {
                ...
                if ( args[0].equals("-?") || args[0].equals("-h")) {
                    System.out.println(
                            "Syntax:\n" +
                                    ...
                                    "\t-ps  {i|a|n}          - print data sorted by departNumber/fullName/startDate\n" +
                                    "\t-psr {i|a|n}          - print data reverse sorted by departNumber/fullName/startDate\n" +
    ...
    static StaffMember readBook( Scanner fin ) throws IOException {
        return StaffMember.nextRead( fin, bookOut )
                ? StaffMember.read( fin, bookOut ) : null;
    }
        ...
        try ( Index idx = Index.load( idxname );
            ...
            while (( pos = fileBak.getFilePointer()) < fileBak.length() ) {
                ...
                if ( Arrays.binarySearch(poss, pos) < 0 ) { // if not found in deleted
                    long ptr = Buffer.writeObject( file, staffMember, wasZipped[0] );
                    idx.put( staffMember, ptr );
                }
            }
        }
        return true;
    ...
    static void appendFile( Boolean zipped )
        ...
        try ( Index idx = Index.load( idxname );
            ...
            for(;;) {
                StaffMember staffMember = readBook( fin );
                if ( staffMember == null )
                    break;
                idx.test( staffMember );
                long pos = Buffer.writeObject( raf, staffMember, zipped );
                idx.put( staffMember, pos );
            }
        }
    }
    ...
    private static void printRecord( RandomAccessFile raf, long pos )
        ...
        boolean[] wasZipped = new boolean[] {false};
        StaffMember book = (StaffMember) Buffer.readObject( raf, pos, wasZipped );
        if ( wasZipped[0] == true ) {
