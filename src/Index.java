...
public class Index implements Serializable, Closeable {
    ...
    IndexOne2One          tableNumbers;
    IndexOne2One           fullNames;
    IndexOne2One          startDates;

    public void test( StaffMember staffMember ) throws KeyNotUniqueException {
        assert( staffMember != null );
        if ( tableNumbers.contains(staffMember.tableNumber)) {
            throw new KeyNotUniqueException( staffMember.tableNumber );
        }
        if ( fullNames.contains( staffMember.fullName )) {
            throw new KeyNotUniqueException( staffMember.fullName );
        }
    }

    public void put( StaffMember staffMember, long value ) throws KeyNotUniqueException {
        test( staffMember );
        tableNumbers.put( staffMember.tableNumber, value );
        fullNames.put( staffMember.fullName, staffMember.fullName, value);
        startDates.put( staffMember.startDate, value);
    }

    public Index()  {
        tableNumbers   = new IndexOne2One();
        fullNames = new IndexOne2N();
        startDates 	= new IndexOne2One();
    }
